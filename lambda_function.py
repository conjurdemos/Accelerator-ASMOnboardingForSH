  ...
  # returns secret_value dictionary, status_code & message for response_body
        ...
        try:
            ...
            if e.response['Error']['Code'] == 'ResourceNotFoundException':
                status_code = 404
                response_body = json.dumps(f"The requested secret {secret_id} was not found.")
            elif e.response['Error']['Code'] == 'AccessDeniedException':
                status_code = 403
                response_body = json.dumps(f"Access to the requested secret {secret_id} was denied.")
                ...
                status_code = 500 
                response_body = json.dumps(f"The requested secret can't be decrypted using the provided KMS key: {e}")
...
def onboardAccount(cyberark_dict, session_token):
  ...
  # returns status_code == 201 for success, response_body with message
        ...
        status_code = 201
        response_body = json.dumps(f"Account {cyberark_dict['account']} onboarded successfully")
